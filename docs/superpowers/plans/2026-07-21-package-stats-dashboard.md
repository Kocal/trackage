...
- `shared/stats.ts`:
  ...
  - `sumDaily(daily): number` — sum of all daily values (used for the npm all-time `total`; the daily map is merged in the orchestrator with a plain spread `{ ...existing, ...new }`).
  ...
  - `total` source: **npm** = `sumDaily(mergedDaily)` (no cheap all-time endpoint); **Packagist** = authoritative `downloads.total` from `.json`.

### R1 — data model + libs + orchestrator code (supersedes Tasks 2, 4, 5, 7-code; folds in Task 3 repo fixes)

**Files:** `shared/stats.ts` (rewrite), `scripts/lib/npm.ts` (harden + daily helper), `scripts/lib/packagist.ts` (total + new daily fetch), `scripts/fetch-stats.ts` (rewrite for new model + BACKFILL/incremental modes — code only, NO live run), `app/data/projects.config.ts` (repo slugs), + their tests. TDD for the pure logic. `scripts/` is outside `pnpm typecheck` scope, so the orchestrator is validated by R2's live run — but rewrite it here so no committed file references removed symbols.
...
### R2 — full live seed (supersedes Task 7's seed run)

**Files:** `data/history.json` (regenerated by running R1's orchestrator).

Orchestrator behaviour (implemented in R1): per package, npm → `fetchNpmDays` → merge `dailyMap(days)` into existing daily, `total = sumDaily(mergedDaily)`; packagist → merge `fetchPackagistDaily(name, from)` into existing daily, `total = fetchPackagistTotal(name).total`; `stars = fetchStars(repo, token)` for all; write `{ total, stars, daily }` per key.
